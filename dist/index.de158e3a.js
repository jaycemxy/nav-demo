...
function init() {
    ...
    if (hashInLocalStorage) hash1 = hashInLocalStorage;
    return {
        "keys": keys1,
        "hash": hash1
    };
}
function generateKeyboard(keys2, hash2) {
    for(var index = 0; index < keys2['length']; index = index + 1){
        var div = tag('div');
        div.className = 'row';
        main.appendChild(div);
        var row = keys2[index] //第一个数组 第二个数组 第三个数组
        ;
        for(var index2 = 0; index2 < row['length']; index2 = index2 + 1){
            var span = createSpan(row[index2]);
            var button = createButton(row[index2]);
            var img = createImage(hash2[row[index2]]);
            var kbd = tag('kbd');
            kbd.className = 'key';
...
function listenToUser(hash3) {
    document.onkeypress = function(xzkjcnxlkcjlk) {
        var key = xzkjcnxlkcjlk['key'];
        var website = hash3[key];
        console.log(website);
